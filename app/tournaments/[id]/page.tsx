...
export default function TournamentDetailPage() {
  ...
  if (data === null) {
    return (
      <div className="pb-6 md:max-w-2xl md:mx-auto">
        ...
        <Link
          href="/tournaments"
          className="mx-4.5 md:mx-0 block text-center py-3.5 rounded-[14px] border border-line text-text font-semibold text-sm"
        >
          Back to tournaments
        </Link>
        <BottomNav />
  ...
  return (
    <div className="pb-6 md:max-w-4xl md:mx-auto">
      ...
      <div className="px-4.5 md:px-0 pt-2">
        <div className="bg-card border border-line rounded-2xl p-4">
          ...
          {champion && (
            <div className="mt-4 flex items-center gap-2 bg-(--gold-dim) border border-gold rounded-xl px-3.5 py-2.5 text-[13px] font-bold text-gold">
              🏆 Champion: {champion}
            </div>
          )}
        </div>
      ...
      <div className="px-4.5 md:px-0 pt-6 pb-2 text-[12px] tracking-wider text-text-dim uppercase font-semibold">
        Fixture
      </div>
      ...
      {participants.length === 0 ? (
        <div className="px-4.5 md:px-0 text-[13px] text-text-dim pb-4">
          No one has joined yet.
        </div>
      ) : (
        <div className="px-4.5 md:px-0 grid grid-cols-2 md:grid-cols-3 gap-2">
          {participants.map((p) => (
            <div
              key={p.id}
